...
class Nip44v2 {
  ...
  static Uint8List hkdfExtract(Uint8List salt, Uint8List ikm) {
    final hmac = HMac(_digest, _hmacBlockSize)..init(KeyParameter(salt));
  ...
  static Uint8List pad(String plaintext) {
    ...
    final prefix = writeU16BE(unpaddedLen);
    final suffix = Uint8List(calcPaddedLen(unpaddedLen) - unpaddedLen);
  ...
  static Future<String> encrypt(
    ...
    final keys = getMessageKeys(conversationKey, nonce);
    ...
    final mac = hmacAad(keys['hmac_key']!, ciphertext, nonce);
...
Future<Map<String, dynamic>> nip44Decrypt(
  ...
  final decoded = jsonDecode(
    await Nip44v2.decrypt(
      content,
      Nip44v2.shareSecret(privateKey, data['pubkey'] as String),
    ),
